...
from apps.lists.services.subtitle_parser_nltk_fast import ConvertTextToSubtitleWords
...
def download_words(request, list_id):
    ...
    words = (
        Word.objects
        .filter(id__in=ids, subtitle_lists=subtitle_list)
    ...
    content = tabulate(
        table,
        headers=["WORD", "TRANSCRIPTION", "TRANSLATION"],
        tablefmt="plain"
    )
    ...
    if request.user.is_authenticated:
        known_word_ids = set(
            KnownWord.objects.filter(
                user=request.user,
                word__subtitle_lists=word_list
...
class ToggleKnownWordView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        known_qs = KnownWord.objects.filter(
            user=request.user,
            word=word
        )
